...
STYLE_FILES="$HOME/.dotfiles/config/waybar_config/style.css"

trap "killall waybar" EXIT

while true; do
	echo -i "$0: Starting waybar in the background..."
        exec waybar -c ~/.dotfiles/config/waybar_config/config -s ~/.dotfiles/config/waybar_config/style.css &
        echo -i "$0: Started waybar PID=$!. Waiting for modifications to ${CONFIG_FILES};${STYLE_FILES}..."
        inotifywait -e modify ${CONFIG_FILES} ${STYLE_FILES} 2>&1 | logger -i
